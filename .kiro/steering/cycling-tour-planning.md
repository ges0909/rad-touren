...
All tour files live under `cycling/`:
...
## Tour Catalog Index (`cycling/README.md`)
...
3. **Save GPX** to `cycling/gpx/{name}.gpx`.
...
6. **Render map** via `render_gpx_map` with curated POI markers (~15–25, deduplicated). Save to `cycling/img/{name}.png`.
7. **Render elevation** via `render_elevation_profile`. Save to `cycling/img/{name}-elevation.png`.
...
12. **Write tour markdown** to `cycling/{name}.md` following the template exactly.
13. **Update index** — append row to `cycling/README.md`.
